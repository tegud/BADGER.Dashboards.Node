	...
	TLRGRP.BADGER.Dashboard.Components.ProviderSummary = function (configuration) {
		...
		var callbacks = {
			success: function (data) {
				...
				var summaryViewModel = {
					...
					tiers: _.map(groupedData, function(tier) {
						...
						if(_.every(tier.providers, function(provider) {
							...
							status = '<div><div class="fa fa-check connectivity-service-status-tier-big-ok-indicator"></div><div class="connectivity-service-status-tier-big-ok-text">OK</div></div>';
						}
						else {
							var checkSummaries = _.map(checkStates, function(state, key) {
								return _.defaults({}, state, {
				...
				overallDescription.html(Mustache.render('<h2>Providers</h2>Description of status goes here.', {}));
